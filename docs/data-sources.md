...
CapWages sometimes uses a player's common name rather than the official NHL first name in its URL (for example `sam-montembeault`). When the direct official-name URL returns 404, the importer reads the public CapWages sitemap, requires an unambiguous surname/name similarity match, and then records the resolved player URL as provenance. The import fails its quality gate if any active NHL roster player still lacks a positive 2025-26 cap hit or contract end year.
