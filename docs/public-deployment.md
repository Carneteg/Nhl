...
Repositoryt innehåller `render.yaml`, `Dockerfile` och `scripts/start-public.sh`. Skapa en ny **Blueprint** i Render och anslut repositoryt. Render bygger containern, monterar en beständig 2 GB-disk och använder `/health` för hälsokontroll. Tjänsten får därefter en stabil publik `https://<service>.onrender.com`-adress.

Den första starten hämtar 32-lagsbaslinjen och kontrakt med cache/rate limiting, skapar en ren Edmonton-simulation och kan därför ta flera minuter. Senare omstarter återanvänder SQLite-databasen på den beständiga disken.
